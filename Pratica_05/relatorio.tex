...
Esta pr\'atica trata dos filtros IIR (filtros recursivos), nos quais a sa\'ida
depende tanto das amostras de entrada quanto de amostras anteriores da pr\'opria
sa\'ida. S\~ao implementadas quatro fam\'ilias de filtros: o passa-baixas e o
passa-altas de p\'olo simples (an\'alogos a um circuito R-C) e os filtros
recursivos de banda estreita (passa-banda e rejeita-banda). Em seguida, esses
conceitos s\~ao aplicados a um problema real de processamento de som: a
recupera\c{c}\~ao de um sinal de voz cujo espectro foi invertido por modula\c{c}\~ao
e contaminado por ru\'ido. Todas as rotinas foram implementadas em MATLAB e os
gr\'aficos reproduzem as telas obtidas em laborat\'orio.

% ============================ ITEM 1 ============================
\section{Filtros de p\'olo simples (R-C): passa-baixas e passa-altas}
\textbf{1) Implemente rotina MATLAB para calcular coeficientes de filtros de
P\'olo simples (R-C) passa-baixas e passa-altas. Voc\^e escolhe fc. Encontre os
gr\'aficos da resposta ao impulso (1 0 0 0 ... 100 pontos) e das respostas em
frequ\^encia (ganho, atenua\c{c}\~ao e fase).}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f1_fpb.png}
\caption{FPB de p\'olo simples ($f_c=0{,}10$): resposta ao impulso, ganho,
atenua\c{c}\~ao (dB) e fase, calculados sobre 100 pontos.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f1_fpa.png}
\caption{FPA de p\'olo simples ($f_c=0{,}10$): resposta ao impulso, ganho,
atenua\c{c}\~ao (dB) e fase.}
\end{figure}
...
\textbf{2) Elabore um sinal com baixa frequ\^encia + alta frequ\^encia e teste os
filtros FPB e FPA calculados com as rotinas anteriores do item 1). Mostre o sinal
gerado e os sinais filtrados (no tempo e na frequ\^encia).}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figs/f2_teste.png}
\caption{Sinal de 30~Hz + 350~Hz e seus resultados ap\'os o FPB e o FPA, no tempo
e na frequ\^encia. Os picos em torno dos \'indices 650 e 970 s\~ao o espelhamento
da FFT em torno de $F_s/2$.}
\end{figure}
...
\textbf{3) Implemente rotina MATLAB para calcular coeficientes de filtros Banda
estreita (passa-banda e corta-banda). Voc\^e escolhe fc e BW. OBS: BW deve ser
estreito! Encontre os gr\'aficos da resposta ao impulso e das respostas em
frequ\^encia (ganho, atenua\c{c}\~ao e fase).}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f3_bp.png}
\caption{Passa-banda ($f_c=0{,}25$, $BW=0{,}008$): resposta ao impulso (senoide
amortecida), ganho, atenua\c{c}\~ao (dB) e fase.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f3_br.png}
\caption{Rejeita-banda ($f_c=0{,}25$, $BW=0{,}008$): resposta ao impulso, ganho,
atenua\c{c}\~ao (dB) e fase.}
\end{figure}
...
\textbf{4) Elabore um sinal com v\'arias frequ\^encias a sua escolha,
compat\'ivel com as frequ\^encias de corte dos filtros do item 3). Teste os
filtros passa-banda e rejeita-banda, banda estreita. Mostre o sinal gerado e os
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figs/f4_teste.png}
\caption{Sinal com cinco frequ\^encias e seus resultados ap\'os o passa-banda e o
rejeita-banda centrados em 250~Hz, no tempo e na frequ\^encia.}
\end{figure}
...
\textbf{5) Demodule o sinal e filtre os ru\'idos, encontrando um sinal o mais
parecido poss\'ivel com o original. Qual a frase demodulada e filtrada obtida?}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f5_modulado.png}
\caption{Sinal recebido (ru\'ido modulado), no tempo e na frequ\^encia. O espectro
\'e dominado pelo ru\'ido branco que ocupa quase toda a faixa.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f5_demodulado.png}
\caption{Sinal ap\'os a demodula\c{c}\~ao por $\cos(\pi n)$. O espectro foi
revertido: o conte\'udo de voz reaparece em baixas frequ\^encias.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f5_passabaixas.png}
\caption{Sinal ap\'os o passa-baixas WindowSinc. O ru\'ido branco de alta
frequ\^encia foi praticamente eliminado, restando a voz e um tom estreito.}
\end{figure}
...
passagem soma a sua atenua\c{c}\~ao (em dB), produzindo um entalhe profundo sem a
necessidade de um filtro de ordem excessivamente elevada.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figs/f5_notch.png}
\caption{Espectro em torno de 766~Hz antes e depois do rejeita-banda aplicado
tr\^es vezes. A raia do tom interferente \'e praticamente eliminada.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figs/f5_final.png}
\caption{\'Audio recuperado (ap\'os demodula\c{c}\~ao, passa-baixas e
rejeita-banda em cascata), no tempo e na frequ\^encia. Os surtos de amplitude
correspondem \`as s\'ilabas da frase falada.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{figs/f5_espectrograma.png}
\caption{Espectrograma do \'audio recuperado. As estruturas de formantes
variando no tempo confirmam a recupera\c{c}\~ao da voz.}
\end{figure}
...
resposta ao impulso, bem como o comportamento suave das curvas de ganho,
atenua\c{c}\~ao e fase de um sistema de primeira ordem. Os filtros de banda
