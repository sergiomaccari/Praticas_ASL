...
subplot(2,2,2);
plot(abs(DFT(x1)), '.-'); title('DFT x1 (150 pontos)');
...
subplot(2,2,1);
plot(fpf, '.-'); title('Resposta ao Impulso Original (hFPF)');

% Curva de Ganho (abs(DFT))
subplot(2,2,2);
plot(abs(DFT(fpf)), '.-'); title('Ganho Original');

% 3.b) Preencher fpf com zeros até 1000 pontos
fpf_zp = [fpf(:)', zeros(1, 1000 - length(fpf))];

subplot(2,2,3);
plot(fpf_zp, '.-'); title('hFPF com preenchimento com zeros');

subplot(2,2,4);
plot(abs(DFT(fpf_zp)), '.-'); title('Ganho com preenchimento com zeros');